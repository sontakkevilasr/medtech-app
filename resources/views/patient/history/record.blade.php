...
@section('title', 'Visit — '.$record->visit_date->format('d M Y'))
@section('page-title')
    <a href="{{ route('patient.history.index') }}" style="font-size:.85rem;font-weight:400;color:var(--txt-lt);text-decoration:none">History</a>
...
<div style="display:flex;flex-direction:column;gap:16px">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:8px;margin-top:16px;padding-top:14px;border-top:1px solid var(--warm-bd)">
            ...
            <div style="display:flex;align-items:center;gap:6px;padding:6px 12px;border-radius:9px;background:var(--parch);border:1px solid var(--warm-bd)">
                ...
                <div>
                    ...
                    <div style="font-size:.875rem;font-weight:600;color:var(--txt)">{{ $val }}{{ isset($vitalUnits[$key]) && $vitalUnits[$key] ? ' '.$vitalUnits[$key] : '' }}</div>
                </div>
            </div>
            @endif
            @endforeach
    ...
    <div class="panel" style="padding:16px 20px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
            ...
            <a href="{{ route('patient.history.prescription.pdf', $record->prescription->id) }}"
               style="font-size:.75rem;padding:4px 12px;background:var(--plum);color:#fff;border-radius:7px;text-decoration:none;font-weight:600">
                ↓ Download PDF
            </a>
        </div>
        <div style="font-weight:600;color:var(--txt);font-size:.9rem;margin-bottom:8px">{{ $record->prescription->prescription_number }}</div>
        ...
        @if($record->prescription->diagnosis_summary)
        <div style="font-size:.78rem;color:var(--txt-lt);margin-top:8px">{{ $record->prescription->diagnosis_summary }}</div>
        @endif
    </div>
    @endif
    ...
    <div class="panel" style="padding:16px 20px">
        ...
        <div style="display:flex;align-items:center;gap:10px;padding:9px 12px;background:var(--parch);border-radius:9px;margin-bottom:6px;border:1px solid var(--warm-bd)">
            ...
            <a href="{{ Storage::disk('public')->url($att['path']) }}" target="_blank" download="{{ $att['name'] }}"
...
<div style="position:sticky;top:78px;display:flex;flex-direction:column;gap:13px">
    ...
    <div class="panel" style="padding:15px 17px">
        ...
        @foreach(['Date' => $record->visit_date->format('d M Y'), 'Doctor' => 'Dr. '.($record->doctor?->profile?->full_name ?? '—'), 'Specialty' => $record->doctor?->doctorProfile?->specialization ?? '—', 'Added' => $record->created_at->diffForHumans()] as $k => $v)
        <div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--warm-bd);font-size:.78rem">
            ...
            <span style="color:var(--txt-md);font-weight:500;max-width:140px;text-align:right;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{{ $v }}</span>
        </div>
        @endforeach
    </div>
